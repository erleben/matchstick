#ifndef UTIL_WRITE_MATLAB_PROFILING_DATA_H
#define UTIL_WRITE_MATLAB_PROFILING_DATA_H
...
namespace util
{
   ...
   *              in matlab syntax.
   */
  inline std::string write_matlab_profiling_data()
  {
...

    {
      ...
      for( ; m != end; ++m)
      {
        ...
        output <<  write_matlab_vector( name, values ) << std::endl;
      }
    }
...

    {
      ...
      for( ; m != end; ++m)
      {
        ...
        for (unsigned int idx = 0u; idx < N; ++idx)
        {
          ...
          output <<  write_matlab_vector( values ) << std::endl;
        }
        output << "};" << std::endl;
...
// UTIL_WRITE_MATLAB_PROFILING_DATA_H
#endif
