#ifndef TINY_PRIME_NUMBERS_H
#define TINY_PRIME_NUMBERS_H

#include <tiny_constants.h>  // Needed for tiny::highest
#include <tiny_random.h>

#include <cmath>            // Needed for floor and sqrt
...
namespace tiny
{
  ...
  inline bool pseudo_prime(int n)
  {
    if((n==1)||(n==2))//--- trivial cases
      return true;
    if(detail::modular_exponentiation(2,n-1,n)!=1)
  ...
  inline int prime_search(int n)
  {
    ...
    if((o%2)==0)
      o++;
    int max_val = tiny::detail::highest<int>();
    for(;o<max_val;o=o+2)
    {
      //      if(trial_division(o))
      //      if(pseudo_prime(o))
